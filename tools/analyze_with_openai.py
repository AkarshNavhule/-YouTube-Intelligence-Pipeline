"""
Stage 3: Intelligence engine via OpenAI.

Sends YouTube video stats, channel data, and transcript text to
GPT-4o (latest OpenAI model) and returns structured content intelligence.
    ...
    "top_videos": [
        {
            ...
            "why_its_working": "string - analysis of why this video performed well"
        }
    ],
    "channel_rankings": [
        {
...
def build_prompt(videos: list, channels: list, transcripts: list, days_back: int) -> str:
    """Build the user message with all YouTube data for the model to analyze."""
...
- For top_videos, explain specifically WHY each video performed well
- For channel_rankings, base growth_signal on subscriber-to-view ratios
- For content_gaps, identify specific topics underrepresented given current interest
- For strategic_recommendations, make them specific and actionable
...
def main():
    print("[Stage 3/6] Analyzing with  via OpenAI...")
    ...
    for f in [INPUT_VIDEOS, INPUT_TRANSCRIPTS]:
        ...
        if f.stat().st_size == 0:
            raise ValueError(f"{f} is empty — delete it and re-run the stage that creates it")
    ...
    def call_openai(strict_json: bool = False) -> str:
        sys = system_msg
        if strict_json:
            sys += " CRITICAL: Respond with ONLY valid JSON. No markdown, no code blocks. Start with { and end with }."
    ...
    try:
        ...
        print("  JSON parse failed, retrying without JSON mode...")
        raw_text2 = call_openai(strict_json=True)
        cleaned2 = raw_text2.strip() if raw_text2 else ""
